...
export const axiosApiCall = async (axiosParams) => {
  const { method, url, data, isPrivate = false, useRefreshToken = false } = axiosParams;

  const token = useRefreshToken ? localStorage.getItem("refreshJWT") : sessionStorage.getItem("accessJWT");
